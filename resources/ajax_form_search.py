    ...
    def get(self):
        ...
        if search_args["field_name"] == "imp":
            ...
            for query_result in query_results:
                results.append(
                    "{}: {}".format(query_result.icd10, query_result.description)
                )
        ...
        # Patient search
            ...
            for query_result in query_results:
                label = "HN #{} - {} -- Clinic ID: {}, Nationality: {}, ID/Passport: {}, NAP: {}, Referred From: {}".format(
                    query_result.hn,
                    query_result.name,
            ...
            distinct_results = (
                model.query.filter(column.ilike("%{}%".format(search_args["query"])))
                .distinct()
                .limit(app.config["MAX_SEARCH_RESULT"])
                .all()
            )
            ...
            for distinct_result in distinct_results:
                results.append(getattr(distinct_result, search_args["field_name"]))
